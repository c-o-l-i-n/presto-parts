...
describe('GeneratePage', () => {
	...
	it('should not render when page not active', () => {
		cleanup()
		const result = render(
			<ActivePageContext.Provider value={{ state: Page.SEPARATE }}>
				{component}
			</ActivePageContext.Provider>
		)
		...
		beforeEach(() => {
			...
			component = (
				<GeneratePage
					...
					songFoldersLocation={undefined}
					songList={undefined}
					instrumentPartsList={undefined}
				/>
			)
			renderResult = render(
				<ActivePageContext.Provider value={{ state: Page.GENERATE }}>
	...
	it('should have "Choose Folder" button', () => {
		const chooseFileButton = screen.queryByText('Choose Folder', {
			selector: 'button',
		})
	...
	describe('after choosing a file with the "Choose File" button', () => {
		...
		beforeEach(async () => {
			...
			chooseFileButton = screen.getByText('Choose Folder', {
				selector: 'button',
			})
